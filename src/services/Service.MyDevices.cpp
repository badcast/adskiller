...
MyDeviceService::MyDeviceService(QObject *parent) : Service(None, parent), mInternalData(0), table(nullptr), dateEditBegin(nullptr), dateEditEnd(nullptr), refreshButton(nullptr)
{
...
{
    if(!canStart() || isStarted() || !MainWindow::current->accessUi_myDevices(table, dateEditBegin, dateEditEnd, refreshButton) ||
        !(table && dateEditBegin && dateEditEnd && refreshButton))
        return false;
    ...
    QObject::disconnect(refreshButton, &QPushButton::clicked, this, &MyDeviceService::sRefresh);
    QObject::connect(refreshButton, &QPushButton::clicked, this, &MyDeviceService::sRefresh);
    sRefresh();
    return true;
}
